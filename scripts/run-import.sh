...
echo "==> [1/5] Running osm2pgsql import + post-import SQL (barrelman-db container)..."
docker exec \
...
echo "==> [2/5] Generating abbreviation index (barrelman container)..."
docker exec \
...
echo "==> [3/5] Populating codes from OSM tags (barrelman container)..."
docker exec \
  -e DATABASE_URL="postgresql://barrelman:${DB_PASS}@barrelman-db:5432/barrelman" \
  barrelman bun run import/generate-codes.ts

echo "==> [4/5] Resolving parent boundary context (barrelman-db container)..."
docker exec barrelman-db psql "$DB_URL" -f /app/import/resolve-parent-context.sql

echo "==> [5/5] Rebuilding tsvector with abbreviations + categories + parent context (barrelman-db container)..."
