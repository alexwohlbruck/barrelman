...
your own backend talking to its own instance. They authenticate differently on
purpose. (On the hosted deployment that second caller is Parchment, the map app
barrelman was built for; on a self-hosted instance it is whatever you point at
it.)
