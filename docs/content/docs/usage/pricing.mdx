...
Existing subscribers keep the price they signed up at until it is changed with
the payment provider; the *allowance* changes immediately for everyone, because
it is read from the code on every request.
