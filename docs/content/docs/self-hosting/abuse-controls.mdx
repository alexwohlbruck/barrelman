...
Two callers take none of this: an account on the `first-party` plan, and — for
the per-minute layers — tiles, which are counted separately. Both are below.
...
### Tiles are counted separately

Every per-minute figure above is shaped for API calls: one request, one answer.
A map is not that shape. One viewport is thirty to sixty tiles, so a plan's
limit is a few seconds of panning, and what the user sees is a basemap arriving
in patches as tiles are refused and retried.

So tile requests count in a window of their own, at `BARRELMAN_TILE_RATE_MULTIPLIER`
times whatever limit is under test — 20× by default. Their own window rather
than a bigger shared one: on a shared window a single pan spends the budget
every other endpoint is then measured against, and the map takes `/search` down
with it.

The multiple is affordable because a tile is the cheapest thing barrelman
serves, 1 credit against 12–40 for routing and isochrones, so the credit
allowance is already the real budget for tile traffic. This ceiling only has to
stop a scraper, not a map. Lower the multiplier if your instance serves tiles
from somewhere that cannot take the load.

### Your own application

An operator's own application is not a customer. Metering it means billing
yourself, and throttling it means rationing your own users — and it is the
caller the layers above punish hardest, because of its shape rather than its
volume: a server-side proxy puts every one of its users behind one address and
one credential, which is precisely what the per-address backstop is built to
catch.

The `first-party` plan is the answer. It is `internal: true` and
operator-assigned, exactly like `demo`, and it sets `unthrottled: true` — every
per-minute window and the concurrency caps are skipped. Assign it from
`/console/accounts` and give the application an ordinary `brm_live_…` key.

Everything else still applies. Scopes are enforced, origin restrictions are
enforced, suspension is immediate, and usage is recorded at zero credits — so
this traffic appears in the dashboards and in abuse detection like anyone
else's. Unmetered is not invisible.

<Callout type="warn">
  Two things are deliberately *not* the mechanism here.

  **Not the shared secret.** `BARRELMAN_API_KEY` could have been exempted
  instead, and that is worse in every way that shows up when something goes
  wrong: it is anonymous in every dashboard, it cannot be revoked without
  breaking every other holder of it, it cannot be scoped, and there is only
  ever one.

  **Not the `Origin` header.** It is written by the caller, so granting on it
  means `curl -H 'Origin: https://your-app'` gets unlimited access. Origins are
  used elsewhere in barrelman only to *narrow* a key, never to grant — and a
  server-side proxy sends none at all.
</Callout>

### Per-visitor

Only for plans that set `requestsPerMinutePerIp` — today just `demo`, the
operator-assigned plan behind the public demo. The tile multiplier applies here
too, so `demo`'s 240/min per visitor is 4,800/min of tiles.
...
Only the refusals a correct client cannot avoid count: an unknown key, a scope
it does not hold, exhausted credits. **A rate-limit 429 is not a strike.** That
response carries a `Retry-After` and a caller that obeys it succeeds, so
counting it would escalate an ordinary burst into a lockout — and since the
lockout answers 429 too, a client that merely retried would keep itself locked
out.

Strikes attach to the **account at the address that earned them**, so rotating
keys sheds nothing while one wedged client cannot refuse the account's other
traffic. Where no account is known, the address alone.
...
An administrator can suspend an account from `/console/accounts`. Opening an
account there shows what it has spent — the current cycle broken down by
endpoint group and by key, plus a lifetime total, because an account that was
abusive last month reads as spotless through a cycle window — along with its
keys and everything ever recorded against it.
...
Suspension is the reversible option and the one that keeps a record. Deletion
keeps nothing: the account's API keys, usage counters, credit ledger, abuse
signals and its own moderation history all go with it, by `ON DELETE CASCADE`.
Afterwards there is no evidence the account existed, which is the point when
someone asks to be removed and the reason it is the wrong tool for a
troublesome customer.

The console asks the operator to type the address back, and the API checks that
confirmation too — an admin-scoped key reaches the route with a one-line curl:

```bash
curl -X DELETE https://your-instance/admin/users/<id> \
  -H "authorization: Bearer $BARRELMAN_ADMIN_SCOPED_KEY" \
  -H "content-type: application/json" \
  -d '{"email":"them@example.com"}'
```

Two accounts are refused: your own, and the last remaining administrator —
with no shared admin secret, an instance with zero admins can only be recovered
from psql. Delete an administrator by demoting them first.

The only trace left is a line in the server log, since the audit trail is one
of the things being erased:

```
[moderation] deleted account usr_… <them@example.com> by usr_…
```
