...
Two callers take none of this: the shared service credential, and — for the
per-minute layers — tiles, which are counted separately. Both are below.
...
`BARRELMAN_API_KEY` is not throttled at all.

It is not a customer — it is the operator's own backend, it is already
unmetered, and the only thing a limit on it bounds is how fast the product it
powers may work. It is also the caller the layers above punish hardest, because
of its shape rather than its volume: Parchment proxies every user's map through
one server, so the whole product arrives on one address under one credential,
and it was being measured against the anonymous per-address ceiling — a limit
sized for a scraper in an open deployment.

Set `BARRELMAN_SERVICE_RPM` to put a ceiling back. It is unset by default, and
the tradeoff to weigh is that the secret is shared: anyone holding it gets the
same exemption.
