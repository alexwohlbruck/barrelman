...
Step 7 is the one exception to running everything as root: the `pelias` CLI
refuses to run as `0:0`, and `sudo -u` does not satisfy it either. See
...
The GBFS importer fetches **every** system in the global catalog and only then
filters to your bbox, so its cost is unrelated to your region — it walks systems
in Abu Dhabi, Zurich and Darmstadt whatever you imported. Measured end to end on
Colorado: **6 min 48 s**, keeping 191 stations out of 1,336 systems. Run it last,
and skip it entirely unless you need bikeshare — nothing else in the pipeline
depends on it.
