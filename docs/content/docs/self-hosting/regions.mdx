...
Every script that resolves regions also takes a **Regions override** in the
console, applied as `REGIONS` for that run only. Reach for it when the env var
and the console disagree — a region named in `REGIONS` but switched off in the
console makes the resolver refuse, and the override is the way to run an import
without editing `.env` and restarting the ops container.
...
The `gbfs_systems` catalog keeps every system it saw, though — only the
*stations* are filtered. MOTIS is handed the systems inside your regions'
bounding boxes, not the whole catalog: it polls every feed it is given for the
life of the process, and its health endpoint is an AND over all of them, so a
single-metro instance given the catalog would both pay for a thousand live
feeds and report itself permanently unhealthy. A global instance still gets
everything.
...
`GRAPHHOPPER_JAVA_OPTS` sizes the JVM for the graph **build**, not for
serving. At continent scale the build is dominated by landmark preparation —
one sequential single-threaded pass per profile (measured on the full US:
under an hour of import, then 10+ hours of LM preparation, 4h14m of it for
the car profile alone). `prepare.lm.threads` in `graphhopper-config.yml` runs
the profiles concurrently if the heap can hold them simultaneously — see the
comment there. The shipped config memory-maps the finished graph
(`graph.dataaccess.default_type: MMAP` in `graphhopper-config.yml`), so at
query time the graph lives in the kernel page cache, not in heap — a big
graph makes serving want free RAM on the box, not a bigger `-Xmx`. The heap
is consumed while importing the PBF and preparing landmarks, and an
undersized heap dies before GraphHopper logs its version, so the failure
looks like a crash rather than an out-of-memory. The on-disk `graph-cache`
size (`du -sh`) remains a reasonable proxy for the build's heap need — raise
`-Xmx` toward it whenever `REGIONS` grows, and take it back down after the
build if RAM is tight.
...
The graph *build* outgrew the heap (serving memory-maps the graph and is not
the constraint). Raise `-Xmx` in `GRAPHHOPPER_JAVA_OPTS` toward the on-disk
`graph-cache` size and re-run `./scripts/rebuild-graphhopper.sh`.
