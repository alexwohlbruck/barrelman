...
roughly 100 GB. It is sparse, so `du` reports far less than `ls` does, but plan
disk for the full size — a filesystem that cannot hold it will fail the import
partway through. For a city or a state the middle tables are both faster and
very much smaller.
</Callout>
