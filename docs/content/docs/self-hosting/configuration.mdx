---
title: Configuration
description: Every environment variable, grouped by what it affects.
---
...
| `BARRELMAN_SEED_SAMPLE_REGIONS` | _(off)_ | Set to `1` to seed the repo's sample regions (`north-carolina`, `nyc-metro`, `global`) into the regions store on first boot. Off by default: a fresh install starts with an empty Regions page, and you add the region you want with **Add by name**, which searches the Geofabrik index. Sample regions on a real instance are clutter in the console and a mis-clickable target on the Scripts page |
...
`REGIONS` is consumed by the **importers**, which run in `barrelman-ops`, and
also by the API — `scripts/rebuild-motis.sh` generates `motis/config.yml` with
`docker exec barrelman`, so the selection is resolved inside the API container.
It is named in both services' `environment:` blocks, and they must agree.

Leaving it unset does not mean "no regions": it falls back to
`north-carolina,nyc-metro`, the dev pair. On a stack importing anything else
that quietly builds the wrong extent, and on one that has since disabled either
region the MOTIS rebuild fails part-way with `Region "…" is disabled`.
...
The Commons Clause in [LICENSE](https://github.com/alexwohlbruck/barrelman/blob/main/LICENSE) removes the right to sell
Barrelman, so a self-hosted instance may not charge third parties for access —
see [LICENSING.md](https://github.com/alexwohlbruck/barrelman/blob/main/LICENSING.md). Subscription billing is gated on a license
...
See [abuse controls](/self-hosting/abuse-controls) for what these mean.
...
| `BARRELMAN_IP_RPM` | `3000` | Floor for the per-address ceiling. An authenticated caller gets this or its plan's own limit, whichever is higher |
| `BARRELMAN_PER_KEY_SHARE` | `0.8` | Share of the account budget one key may spend |
| `BARRELMAN_TILE_RATE_MULTIPLIER` | `20` | Multiple of every per-minute limit that tiles get, in a window of their own. One map view is 30–60 tiles |
| `BARRELMAN_SERVICE_RPM` | unset | Ceiling on `BARRELMAN_API_KEY`, which has none by default — it is the operator's own backend, not a customer |
...
| `BARRELMAN_ABUSE_STRIKES` | `25` | Unavoidable rejections (bad key, missing scope, no credits) before the penalty box. Rate-limit 429s do not count |
...
| `BARRELMAN_SEARCH_ADDRESS_BUDGET_MS` | `2500` | How long `/search` waits for Pelias address results before returning POIs without them. Pelias answers healthy queries in well under a second; this stops a slow or unreachable geocoder from holding the whole search up to its 10s hang backstop |
| `BARRELMAN_SEARCH_TRIGRAM_BUDGET_MS` | `2500` | How long `/search` waits for the fuzzy trigram layer before answering with the precise layers alone. That layer supplies typo tolerance and reads roughly 215 MB of index per call — fast against a warm index, and on an instance whose table dwarfs RAM slow enough that `BARRELMAN_STATEMENT_TIMEOUT_MS` cancels it, so the caller waits the whole timeout for nothing. `0` waits the full statement timeout |
| `BARRELMAN_TRANSIT_ROUTE_MIN_RANK` | `0.5` | Lowest match score a GTFS route may have and still be returned by `/search`. Route matching includes a loose trigram branch so phrases like "harlem line" reach Metro-North's "Harlem"; across a large feed corpus that branch also pairs unrelated queries with whatever route shares a few trigrams, and routes outrank places in the result merge, so those accidents evict real answers. Exact short-name hits score 0.95, substring matches 0.6-0.8 |
...
`GRAPHHOPPER_JAVA_OPTS` is the one to watch as `REGIONS` grows, but it sizes the
graph **build**, not serving. The shipped config memory-maps the finished graph
(`graph.dataaccess.default_type: MMAP`), so a query-time graph lives in the
kernel page cache and a big one wants free RAM on the box rather than a bigger
`-Xmx`. Heap is consumed importing the PBF and preparing landmarks, and an
...
| `OSM2PGSQL_FLAT_NODES` | — | Path to a flat node file. Unset keeps node coordinates in Postgres. Set it for continent- and planet-sized imports — see below |
| `OSM2PGSQL_CACHE_MB` | osm2pgsql's own (800) | Node cache in MB. Ignored when `OSM2PGSQL_FLAT_NODES` is set |
| `OSM2PGSQL_PROCESSES` | osm2pgsql's own | Parallel workers for the middle tables |
| `IMPORT_ENGINE_OVERLAP` | `auto` | Whether the GraphHopper and basemap builds run concurrently with the database import. Both read only the PBF, so overlapping saves hours — but GraphHopper's heap alongside Postgres OOM-kills it on a small box. `auto` overlaps only at ≥48 GB RAM; `0` builds the engines after the import (GraphHopper first, then basemap); `1` forces overlap |

### Node storage on large imports

osm2pgsql has to remember every node's coordinates so it can assemble ways from
them later. By default those go in the `planet_osm_nodes` middle table, and
building a way is an index lookup per node. That is fine while the table fits in
memory. It stops being fine somewhere around a country: the table outgrows RAM,
every lookup becomes a disk seek, and the import goes from hours to days.

`OSM2PGSQL_FLAT_NODES` moves node storage to a flat file addressed directly by
node ID — one seek, no index — and skips `planet_osm_nodes` altogether.

```dotenv
OSM2PGSQL_FLAT_NODES=/data/flat-nodes.bin
```

<Callout type="warn" title="Not a free win below continent scale">
The file is sized by the **highest node ID in the input**, not by how many nodes
you keep. Any extract carrying present-day OSM IDs therefore produces a file of
roughly 100 GB — a measured 106 GB for North America, and osm2pgsql writes it
dense, so all of that is real disk. A filesystem that cannot hold it will fail
the import partway through. For a city or a state the middle tables are both
faster and very much smaller.
</Callout>

The same value is read by `update-osm.sh`, so daily diffs use the node storage
the import created. Do not change or delete the path afterwards: the osm2pgsql
version `barrelman-db` ships does not record it in the database, so an append
that cannot find the file looks for coordinates in a `planet_osm_nodes` that was
never written, resolves every way in the diff to nothing, and deletes real
geometry rather than failing. If you need to change it, re-import from scratch.
...
[accounts & API keys](/usage/accounts#unmetered-keys-and-why-nobody-can-mint-one).
...
| `BARRELMAN_DB_MAX_WAL_SIZE` | `4GB` | `16GB` on an import-heavy box — the stock 1GB checkpoints per gigabyte of WAL, which during an import is constant, and each checkpoint re-arms full-page writes |
| `BARRELMAN_DB_CHECKPOINT_TIMEOUT` | `15min` | `15min` |
| `BARRELMAN_DB_WAL_COMPRESSION` | `on` | `on` |
| `BARRELMAN_DB_MAINT_WORKERS` | `4` | `8` — what the bulk index builds at the end of a full import scale by |
| `BARRELMAN_DB_WAL_LEVEL` | `replica` | `minimal` if (and only if) nothing replicates from or archives WAL off this database; index builds then skip WAL entirely. Requires `BARRELMAN_DB_MAX_WAL_SENDERS=0` — set both or Postgres refuses to start |
| `BARRELMAN_DB_MAX_WAL_SENDERS` | `10` | `0` alongside `wal_level=minimal` |
