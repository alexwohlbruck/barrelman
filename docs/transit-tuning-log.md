...
| 24 | **Junction-anchored merge-window start** | A joining line now bundles FROM the junction where it physically meets the shared corridor, not a block past it. Two late-start residuals fixed: the Broadway-Lafayette F/M local ran a ~250 m unmerged orange stub beside the B/D express before the bundle onset (round-22's `family_sustained_min_m` 450→300 fix left it just under the floor), and the Chicago Clark/Lake Blue subway ran a ~290 m coincident-but-separate stub under the Loop elevated before joining. When a merge window reaches an end where the joining corridor TERMINATES on the partner (a shared junction node), the family ramp/sustained-length gates and the cross-family `frac_below` kiss gate are judged on the window itself instead of a fixed length / symmetric neighbourhood — the bundle onsets at the junction. Anti-kiss gates unchanged (a kiss is never junction-anchored). Bway-Laf dup-run 251.6→~0 m (F/FX vs B/D/M); Chicago Blue stub 289.6→0 m. | family ramp exemption + `ends_ok` scoped to junction-anchored windows (`_try_merge`); new `cross_family_junction_frac` 0.90 (junction-anchored cross window judged on window frac_below, not the grown context); `CONFIG_FORMAT_VERSION` 15→16 |
| 26 | **Stable ribbon offsets (straight through-lines at junctions)** | The symmetric offset `(slot-(count-1)/2)*gap` RE-CENTERED the whole bundle whenever `line_count` changed at a junction, so a straight through-line's perpendicular offset shifted and a transition eased that shift — curving the ribbon on dead-straight track (nyc:subway-v3 z15: ~40 no-op transitions + ~46 half-slot re-center shifts on essentially-straight track). Fix: re-anchor each corridor's ribbons by one additive SHIFT (slot ORDER untouched — no braiding, anti-kiss gates intact) so a line running STRAIGHT through a junction (its bundle merely gains/loses a neighbour at the edge) holds a constant offset; only the joining/leaving line eases. Solved EXACTLY per straight-joint coupling component (bent joints don't curve a constant-offset ribbon, so they don't couple — components stay ≤11 corridors) by backtracking over the 5-value half-gap grid, bounded to 1 gap off centreline (asymmetric bundle growth — the bundle sits slightly off-centre, the accepted Apple tradeoff), with a HARD MONOTONE guard: no joint's offset delta may exceed its symmetric baseline, so re-anchoring only REMOVES straight-track curving, never adds it. Emitted legacy `(slot,line_count)` re-derived from the authoritative offset via the tile function's half-gap k/h scheme, so `offset_px==(slot-(lc-1)/2)*gap` still holds (stock-Mapbox degradation). No-ops on straight track skip to steady as before. Straight through-line curves: NYC 44→30, CHI 10→7 (all HOLDABLE joints fixed; the residual are genuine mid-bundle express/local reorders that must ease), **zero regressions** both cities. All bundles unchanged (bundling is upstream in linegraph). | `segments.corridors.assign_stable_offsets` (DFS per straight-coupling component); `STRAIGHT_JOINT_DEG` 8, `DEFAULT_DRIFT_CAP_PX` 1×gap; `segments.segment.legacy_slot_count`; `Ribbon.base_offset_px` |
| 27 | **Delancey throat-twin fold** | Near Delancey St-Essex toward the Williamsburg Bridge, short (~162 m) unmerged M vs J/Z parallels fanned at the junction throat while the main J/Z+M bundle (0 m over 4.9 km) was fine: the `[J,M]` and `[M,Z]` fragments (equal {orange,brown} colour families, different route sets) only become co-terminal AFTER the merge passes (ladder contraction/tear-healing unify the wye nodes late), and they bow apart past `family_gap`, so the proximity-gated family pass never paired them. Fix: `_fold_throat_twins` post-pass folds short co-terminal family twins into one union-set ribbon via the existing co-terminal-twin gate, targeted so a real fork is never folded — the union must already continue on a bundle at BOTH shared nodes (a throat inside a bundle, not a divergence). Delancey-area missed-bundle → 0; bundle_audit otherwise unchanged (NYC 1 residual = the Coney Island N/D terminal split); NYC 589→575 emitted edges (fewer spurious fragments); Chicago byte-identical (145 edges); all round-21/24 bundles + O'Hare survive (sites_exam 20/20). | `_fold_throat_twins` + family branch of `_try_merge` co-terminal twin; `twin_throat_max_len_m` 220; `CONFIG_FORMAT_VERSION` 17→18 |
| 25 | **Cache-digest completeness (build reproducibility)** | The way-graph corridor cache digest (`waygraph.waygraph_digest`) hashed only the route SHAPES' lon/lat — NOT route COLOUR (`color_key`, which drives the round-9/21 family + cross-family merges), the per-pattern route_id, or the GTFS STOP positions/names (which round-22 conflation moves and `stations.py` consumes to place station nodes). So a conflation / colour / route change left the digest unchanged and a STALE corridor cache was reused: `chicago:l-v3` diverged to **167 edges** (~22 spurious corridor fragments from the round-21 "transient" topology) while a clean rebuild deterministically produces **145 edges**, and the exam pins had been re-pinned to the stale 167 state. Fix: `waygraph_digest` now folds in, per pattern in load order, route_id + color_key + shape coords, plus the sorted (stop_id, lon, lat, name) set station placement consumes — any input that changes the output now changes the digest. Both cities rebuilt clean; DB == fresh rebuild == pins. Determinism proven: 2 clean-cache rebuilds byte-identical (edge/node/edge_line counts + geom md5) both cities; committed guard `linegraph/tests/test_determinism.py` proves a rebuild reproduces the DB. All user fixes survive the clean rebuild (O'Hare Blue on the OSM platform, Clark/Lake Blue in the 6-line Lake St bundle, Queens Blvd E/F/M/R & DeKalb B/D+N/Q/R/W bundles, Broadway single yellow, anti-hop NYC 178 / CTA 0, bus names conflated). `test_real_emit` made hermetic (throwaway `chicago:l-v3-test` key + teardown) so pytest no longer clobbers the live build. NYC (589 edges) was already deterministic-clean; re-pinned Chicago exams 167→145 edges / 253→223 edge_lines, stability Loop sites 6→5, segments sites 18→13 (17→11 junctions; Ashland composition restored). `chicago:l` LOOM md5-identical. | `CONFIG_FORMAT_VERSION` 16→17; `waygraph_digest(patterns, …)` (was `shapes`); new `_stop_digest_rows` |
